...
def write_stats(device, payload):
    """write to tmp file"""
    fp = open('/tmp/check_tcptraffic_py_%s_%s' % (device,
                                                  getpass.getuser()), 'w')
...
def get_stats(device):
    ...
    for line in open('/proc/net/dev', 'r').readlines():
        ...
        if len(tokens) == 17:
            ...
            payload = dict(valid=datetime.datetime.utcnow(
                ).strftime("%Y-%m-%dT%H:%M:%SZ"), rxbytes=rxbytes,
                           txbytes=txbytes, device=device)
            return payload
...
def main(argv):
    ...
    print(("TCPTRAFFIC OK - %s %.0f bytes/s | TOTAL=%.0fB;400000000;500000000 "
           "IN=%.0fB;; OUT=%.0fB;; TIME=%.0f;;"
           ) % (device, rxrate + txrate, rxrate + txrate, rxrate, txrate,
                seconds))
    sys.exit(0)
