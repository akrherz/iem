...
_CURRENT_RE = "[A-Z] ([0-9]?[0-9][0-9])\s+([0-2][0-9]:[0-9][0-9]) ([0-1][0-9]/[0-3][0-9]/[0-9][0-9])\s+([A-Z]{1,3})\s+([0-9][0-9])(MPH|KTS) ([0-9][0-9][0-9])[F,K] ...F ([0-9\-]{3})F ([0-9][0-9][0-9])% ([0-9]+.[0-9][0-9])([\"RFS]) ([0-9]+.[0-9][0-9])\"D ([0-9]+.[0-9][0-9])\"M"
...
def sendWindAlert(id, alertSPED, alertDrctTxt, myThreshold):
    if id in [904, 75, 907, 918, 9, 913,924,610,619]:
        return
