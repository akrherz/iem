...
def main(argv):
    ...
    for x0, y0 in tqdm(queue, disable=not sys.stdout.isatty()):
        url = (
            ...
            "outputList=NETCDF"
        ) % (sts.strftime("%Y%m%d"), ets.strftime("%Y%m%d"), y0, x0,
        ...
        with ncopen(ncfn) as nc:
            for day, _ in enumerate(nc.variables['time'][:]):
                ...
                # get currentdata
                present = current[
                    date]['data']['power_swdn'].values[jslice, islice]
                if present.mean() == data.mean():
                    continue
                current[
                    date]['data']['power_swdn'].values[jslice, islice] = data
    ...
    for date in current:
        ...
        subprocess.call("python ../iemre/db_to_netcdf.py %s" % (
            date.strftime("%Y %m %d"), ), shell=True)
