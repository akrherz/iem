...
cursor.execute("""SELECT ST_x(geom), ST_y(geom), min(min_tmpf) from
 ...
 s.day > '2015-05-16' and min_tmpf > 0 and t.country = 'US' and
...
m = MapPlot(title='17-21 May 2015 Minimum Temperature Reports', axisbg='white',
            subtitle='Based on NWS Cooperative Observer Data',
            sector='midwest')

x,y = m.map(tuple(x35.x), tuple(x35.y))
m.map.scatter(x,y, marker='o',color='g', s=50, zorder=1, label="Sub 36$^\circ$F")
...
m.map.scatter(x,y, marker='+', color='r', s=50, zorder=2, label="Sub 29$^\circ$F")
