...
opts = {
        ...
        'edate': datetime.date(2014, 1, 1),
        'normalized': False,
        'dbcols': ('TO',),
        }
opts['title'] = 'Yearly Average Minutes'
opts['years'] = opts['edate'].year - opts['sdate'].year
opts['units'] = 'minutes'
...
m = MapPlot(sector='nws',
            title=opts['title'],
            subtitle='%s, period: %s thru %s' % (opts['subtitle'],
...
pcursor.execute("""
 WITH data as (SELECT ugc, count(*) / %s  as data from 
...
data = {}
for row in pcursor:
    data[ row[0] ] = row[ 2 if opts['normalized'] else 1 ]
