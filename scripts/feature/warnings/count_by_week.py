...
def get(state):
    pcursor.execute("""
     ...
     warnings where phenomena = 'SV' and significance = 'W' and substr(ugc,1,2) = %s
...
for i, state in enumerate(['IA', 'OK', 'AL', 'TX']):
    ...
    ax.scatter(np.arange(1,54), counts, marker=markers[i], s=75, label=states[i], c=c[i],
               zorder=i+1)
ax.set_title("1986-2013 Percentage of Years with\n 1+ Severe Thunderstorm Warning for the Week of Year")
ax.set_xlim(0, 53)
