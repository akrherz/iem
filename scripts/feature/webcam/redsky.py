...
while now < ets:
    ...
    sunrise = datetime.datetime.strptime( "%s" % (myloc.next_rising(sun),), '%Y/%m/%d %H:%M:%S')
    sunrise = sunrise.replace(second=0)
    sunrise = sunrise - datetime.timedelta( minutes = sunrise.minute % 10)

    sunset = datetime.datetime.strptime( "%s" % (myloc.next_setting(sun),), '%Y/%m/%d %H:%M:%S')
    sunset = sunset.replace(second=0)
    sunset = sunset - datetime.timedelta( minutes = sunset.minute % 10)

    uri = sunrise.strftime('http://mesonet.agron.iastate.edu/archive/data/%Y/%m/%d/camera/KCCI-026/KCCI-026_%Y%m%d%H%M.jpg')
    try:
    ...
    if redish > 120 and g < 110: # Our arb thresholding
        ...
        cursor.execute("""SELECT sum(p01i) from t"""+ `sunrise.year` +"""
        WHERE station = 'AMW' and valid BETWEEN '%s+00' and '%s+00'""" % (
                        (sunrise + datetime.timedelta(hours=1)).strftime("%Y-%m-%d %H:%M"),
                        (sunrise + datetime.timedelta(days=1)).strftime("%Y-%m-%d %H:%M")))
        row = cursor.fetchone()
        if row[0] >= 0.01:
            print ' ... HIT'
            output.write("1\n")
    ...
    uri = sunset.strftime('http://mesonet.agron.iastate.edu/archive/data/%Y/%m/%d/camera/KCCI-016/KCCI-016_%Y%m%d%H%M.jpg')
    try:
        data = urllib2.urlopen(uri).read()
    except urllib2.HTTPError:
        print 'Download failed'
    ...
    if redish > 120 and g < 110: # Our arb thresholding
        ...
        cursor.execute("""SELECT sum(p01i) from t"""+ `sunrise.year` +"""
        WHERE station = 'AMW' and valid BETWEEN '%s+00' and '%s+00'""" % (
                        (sunset + datetime.timedelta(hours=1)).strftime("%Y-%m-%d %H:%M"),
                        (sunset + datetime.timedelta(days=1)).strftime("%Y-%m-%d %H:%M")))
        row = cursor.fetchone()
        if row[0] >= 0.01:
            print ' ... HIT'
            output.write("1\n")
