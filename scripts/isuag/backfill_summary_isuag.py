...
def two():
    ...
    for row in icursor:
        ...
        if (row2[1] is None or row2[0] is None or row2[2] is None or
                round(row2[0], 2) != round(min_rh, 2) or
                round(row2[2], 2) != round(avg_rh, 2) or
...
def one():
    ...
    for i, row in enumerate(icursor):
        ...
        def myfunc():
            ...
            id = %s) and day = %s
            """, (avg_sknt, max_tmpf, min_tmpf, srad, pday, avg_rh,
                  row[0], row[1]))
        myfunc()
        if iemcursor.rowcount != 1:
            print("update %s %s did not work" % (row[0], row[1]))
            iemcursor.execute("""
            INSERT into summary_"""+str(row[1].year)+""" (iemid, day)
