...
def merge_grib(ts, nc):
    ...
    for fhour in range(6, 169, 6):
        ...
        for grb in grbs:
            if (grb.valid_key('parameterName') and
                    grb['parameterName'] == 'Minimum temperature'):
                # This is 12z
                days = int((fts - ts).total_seconds() / 86400)
                nc.variables['low_tmpk'][days, :, :] = grb.values
            elif (grb.valid_key('parameterName') and
                    grb['parameterName'] == 'Maximum temperature'):
                # This is 0z
                days = int((fts - ts).total_seconds() / 86400) - 1
                nc.variables['high_tmpk'][days, :, :] = grb.values
            elif (grb.valid_key('parameterName') and
                    grb['parameterName'] == 'Total precipitation'):
                # This is tricky
                days = (fhour - 6) / 24
                if days < 0:
                    continue
