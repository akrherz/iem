...
def update(tbl, col, val, yr, station, valid):
    sql = """
        ...
        valid = '%s'
    """ % (tbl, col, val, col, yr, station, valid)
    ...
    for tbl in ['climate51', 'climate']:
        ...
        for row in ccursor:
            ...
            if row['high'] > rec['max_high']:
                update(tbl, 'max_high', row['high'], row['day'].year, stid,
                       dkey)

            if row['high'] < rec['min_high']:
                update(tbl, 'min_high', row['high'], row['day'].year, stid,
                       dkey)

            if row['low'] > rec['max_low']:
                update(tbl, 'max_low', row['low'], row['day'].year, stid,
                       dkey)

            if row['low'] < rec['min_low']:
                update(tbl, 'min_low', row['low'], row['day'].year, stid,
                       dkey)

            if row['precip'] > rec['max_precip']:
                update(tbl, 'max_precip', row['precip'], row['day'].year, stid,
                       dkey)
