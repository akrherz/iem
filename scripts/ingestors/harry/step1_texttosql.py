...
def get_current(year, month):
    ...
    """, PGCONN, params=(year, month), index_col=['station', 'dom'])
...
def main(argv):
    ...
    for linenum, line in enumerate(lines):
        ...
        try:
            ...
            print(("ABORT! timefail yr:%s mo:%s dy:%s linenum:%s"
        ...
        if cursor.rowcount != 1:
            print("ABORT: update not==1, %s %s cnt:%s" % (dbid, ts,
                                                          cursor.rowcount))
            return
        hits += 1
