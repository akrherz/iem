...
def read_inventory():
    ...
    for line in open('ghcnd-inventory.txt'):
        if not line.startswith('USC'):
            continue
        rows.append([line[:11], line[31:35],
                     line[36:40], line[41:45]])
    df = pd.DataFrame(rows,
                      columns=['sid', 'vname', 'start', 'end'])
    df['years'] = (pd.to_numeric(df['end'], errors='coerce') -
                   pd.to_numeric(df['start'], errors='coerce') + 1)
    return df
...
def read_stations(state):
    ...
    for line in open('ghcnd-stations.txt'):
        if not line.startswith('USC') or line[38:40] != state:
            continue
        rows.append([line[:11], line[13:20], line[21:30], line[31:37],
                     line[38:40], line[41:76].strip()])
    df = pd.DataFrame(rows,
                      columns=['sid', 'lat', 'lon', 'elev', 'state', 'name'])
    ...
    for sid, row in df.iterrows():
        ...
        for vname in ['TMAX', 'TMIN', 'PRCP']:
            if vname not in years.index:
                useme = False
                continue
            if (years.at[vname, 'years'] < 50 or
                    maxyear.at[vname, 'end'] < "2010"):
                useme = False
                continue
        ...
        """ % (state, sid[-4:], row['name'], state, state, row['name'],
               row['elev'], row['lon'], row['lat'])
        output.write(sql)
    output.close()
