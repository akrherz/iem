...
ccursor.execute("""
  ...
  """)
for row in ccursor:
    obs[ row[0].strftime("%Y%m%d") ] = {'precip': row[1], 'snow': row[2]}

out = open('step4.csv', 'w')
out.write('apltn_type,actvy_datedb,actvy_type,vis_month,vis_year,entry_ccyy,sem_num,entry_year,hs_status,city,st,zip_5,lat,lon,apply,offer,accept,enroll,coll,vis_order,female,junior,senior,"ames_high","ames_precip","ames_clhigh","ames_clprecip","remote_clhigh","remote_clprecip","wxhigh_score","wxprecip_score",AVGT,RELH,TEMP,SNOW,PRECIP,HEAT,WINDCHILL,CLOUDY\n')

for line in open('step3.csv'):
    tokens = line.split(",")
    if tokens[0] == 'apltn_type':
        continue
    day = datetime.datetime.strptime(tokens[1].replace('"',''), '%m/%d/%Y')
  ...
  """ % (day.strftime("%Y-%m-%d"),day.strftime("%Y-%m-%d")))
    ...
    snow = "N"
    if obs[day.strftime("%Y%m%d")]['snow'] > 0:
        snow = "Y"

    precip = "N"
    if obs[day.strftime("%Y%m%d")]['precip'] > 0:
        precip = "Y"

    out.write("%s,%.1f,%.1f,%s,%s,%s,%s,%s,%s\n" % (line.strip(), avgtmpf, avgrelh,
