...
def run(prod, sts):
    ...
    outdataset = out_driver.Create('max.tiff', n0r.RasterXSize, 
        n0r.RasterYSize, n0r.RasterCount, gdalconst.GDT_Byte)
    # Set output color table to match input
    outdataset.GetRasterBand(1).SetRasterColorTable( n0rct )
    outdataset.GetRasterBand(1).WriteArray( numpy.resize(maxn0r, 
                                (n0r.RasterYSize, n0r.RasterXSize)))
    del outdataset
    ...
    cmd = ("%s -p 'plot a %s0000 bogus GIS/uscomp/max_%s_0z0z_%s.wld "
