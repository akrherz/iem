...
for _, row in df.iterrows():
    ...
    """ % (nwsli, sname, network, country, state, sname, elev,
           lon, lat)

    cursor.execute("""SELECT * from stations where id = %s and network = %s
    """, (nwsli, network))
    if cursor.rowcount == 0:
        cursor.execute(sql)
    else:
        cursor.execute("""UPDATE stations SET geom = 'SRID=4326;POINT(%s %s)',
