...
queryStr = "SELECT id, count(id) as cnt, \
	sum(CASE WHEN pday >= 0 THEN pday ELSE 0 END) as prectot from summary_%s s JOIN stations t on (t.iemid = s.iemid) \
	WHERE date_part('month', day) = date_part('month', CURRENT_TIMESTAMP::date)\
	and pday >= 0 and t.network = 'IA_COOP' and \
	extract(year from day) = %s GROUP by id" % (now.year, now.year,)
