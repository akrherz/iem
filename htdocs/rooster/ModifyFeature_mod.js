 ...
 *  - <OpenLayers.Control>
 */
OpenLayers.Control.ModifyFeature_mod = OpenLayers.Class(OpenLayers.Control, {
    ...
    dragStart: function(feature, pixel) {
		//alert(this.feature.geometry.components[0].components.length);
		this.tmp_componente = this.feature.geometry.components[0].clone();
    ...
    dragVertex: function(vertex) {
         ...
         */
				...
				/*for(i=0; i<componente.components.length-1; i++){
				tmp_line_comp = new OpenLayers.Geometry.LineString([componente.components[i],componente.components[i+1]]);
				text3 += "Line "+(i+1)+": " + tmp_line_comp.toString() + " Diverso: "+diverso+"<br>";
				if ((last.intersects(line_comp) && i!=(componente.components.length-3) && i!=0) || (before_last.intersects(line_comp) && i!=(componente.components.length-4))){
					dointersects = true;
					intersects_i = i+1;
					break;
				}
		...
		if(this.feature.geometry.CLASS_NAME == "OpenLayers.Geometry.Point") {
			...
			if(this.feature.geometry.CLASS_NAME == "OpenLayers.Geometry.Polygon") {
					componente = this.feature.geometry.components[0];
				}
			
			for(i=0; i<componente.components.length-1; i++){
					if(!componente.components[i].equals(this.tmp_componente.components[i])){
						i_before = (i==0)?(componente.components.length-1):i;
						before_last = new OpenLayers.Geometry.LineString([componente.components[i_before-1],componente.components[i_before]]);
						last = new OpenLayers.Geometry.LineString([componente.components[i],componente.components[i+1]]);
						id_punto = i;
						break;
					}
				}
			...
			for(i=0; i<componente.components.length-1; i++){
				i1 = id_punto;
				i2 = (id_punto==0)?(componente.components.length-2):(id_punto-1);
				check1=(i1==componente.components.length-2)?0:(i1+1);
				check2=(i2==0)?(componente.components.length-2):(i2-1);
				if(i!=i1 && i!=i2){
					//alert("Linea: "+i+" - i1: "+i1+" i2: "+i2+" check1: "+check1+" check2: "+check2);
					line_comp = new OpenLayers.Geometry.LineString([componente.components[i],componente.components[i+1]]);
					text += "Line "+(i+1)+": " + line_comp.toString() + "<br>";
					contr1 = (last.intersects(line_comp) && (i!=check1))?true:false;
					contr2 = (before_last.intersects(line_comp) && (i!=check2))?true:false;
					if (contr1 || contr2){
						if(!this.dointersects){
						this.tmp_feature = this.tmpbl_feature.clone();
						}
						this.dointersects = true;
						intersects_i = i;
						text2 = "<br><br>Intersected Line: " + (intersects_i+1);
						break;
					}
					else{
				...
				if(i==(componente.components.length-2)){
					this.dointersects=false;
					this.tmpbl_feature=this.feature.clone();
				}
			}
			
			if(this.dointersects){text2 = "<br><br>Intersected Line: " + (intersects_i+1);}
				//$("componenti").innerHTML = text + text2;
			
			//this.drags = this.drags+1;
    ...
    CLASS_NAME: "OpenLayers.Control.ModifyFeature_mod"
});
