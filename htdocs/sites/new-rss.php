...
$cached_rss = cacheable("newstations_rss", 3600)(function () {
    ...
    $s .= "<link href=\"{$EXTERNAL_BASEURL}/sites/locate.php\" />\n";
    ...
    for ($i = 0; $row = pg_fetch_array($rs); $i++) {
        ...
        $s .= "<author><name>Daryl Herzmann</name><email>akrherz@iastate.edu</email></author>\n";
        $s .= "<link href=\"{$EXTERNAL_BASEURL}/sites/site.php?station=" . $row["id"] . "&amp;network=" . $row["network"] . "\" />\n";
        $s .= "<content type=\"html\">{$cbody}</content>\n";
        $s .= "<id>{$EXTERNAL_BASEURL}/sites/site.php?station=" . $row["id"] . "&amp;network=" . $row["network"] . "</id>\n";
