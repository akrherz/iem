...
if ($opt === "bystation"){
	...
	$col1label = "Date";
	$uri = sprintf("http://iem.local/json/cf6.py?station=%s&year=%s",
		$station, $year);
	...
	$uri = sprintf("http://iem.local/geojson/cf6.py?dt=%s",
		date("Y-m-d", $day));
	$data = file_get_contents($uri);
	$json = json_decode($data, $assoc=TRUE);
	$arr = $json['features'];
...
foreach($arr as $entry){
	...
	if ($byday){
		$link = sprintf("cf6table.php?station=%s&year=%s", $row["station"],
				date("Y", $ts));
		$col1 = sprintf("<a href=\"%s\" class=\"small\">%s %s</a>", $link, $row["station"],
				$nt->table[$row["station"]]['name']);
	}else {
		$link = sprintf("cf6table.php?opt=bydate&year=%s&month=%s&day=%s", date("Y", $ts),
				date("m", $ts), date("d", $ts));
