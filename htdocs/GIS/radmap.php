...
$sql = sprintf("g from (select phenomena, eventid, ST_multi(ST_union(geom)) as g from warnings WHERE significance = 'A' and phenomena IN ('TO','SV') and issue <= '%s:00+00' and expire > '%s:00+00' GROUP by phenomena, eventid ORDER by phenomena ASC) as foo using SRID=4326 using unique phenomena",
...
if ($ts2 > $ts){
 $sql = "geom from (select distinct city, magnitude, valid, geom, type as ltype, city || magnitude || x(geom) || y(geom) as k from lsrs_". date("Y", $ts) ." WHERE valid >= '". gmstrftime("%Y-%m-%d %H:%i", $ts) .":00+00' and valid < '". gmstrftime("%Y-%m-%d %H:%i", $ts2) .":00+00' ORDER by valid DESC) as foo USING unique k USING SRID=4326";
} else {
 $sql = "geom from (select distinct city, magnitude, valid, geom, type as ltype, city || magnitude || x(geom) || y(geom) as k from lsrs_". date("Y", $ts) ." WHERE valid = '". gmstrftime("%Y-%m-%d %H:%i", $ts) .":00+00') as foo USING unique k USING SRID=4326";
}
$lsrs->set("data", $sql);
//echo $sql; die();
$lsrs->draw($img);
