...
if (strlen($network) == 7 || $network == "OT") { // HACK
  while( list($key, $value) = each($cities) ){
    if ($value["network"] == $network) $myStations[$key] = $key;
...
mktitle($map, $img, $ts ." Current ". $network ." ". $varDef[$var] );
