...
def plotter( fdict ):
    ...
    ccursor.execute("""
    SELECT year, extract(doy from day), gdd50(high, low) from """+table+"""
    ...
    ax.plot(np.arange(1,367), avg, color='r', zorder=2, lw=2.,label='Average')
    ax.scatter(np.arange(1,367), obs[year-syear,:], color='b',  zorder=2,
               label='%s' % (year,))
    p5, p25,p75,p95 = np.percentile(obs[:-1,:], [5,25,75,95], 0)
    ax.bar(np.arange(1,367), p95-p5, bottom=p5, ec='tan', fc='tan', zorder=1,
           label='5-95 Percentile')
    ax.bar(np.arange(1,367), p75-p25, bottom=p25, ec='lightblue', fc='lightblue', 
           zorder=1, label='25-75 Percentile')
    ax.set_xlim(1,367)
    ax.set_ylim(-0.25,40)
    ax.grid(True)
    ax.set_title("%s-%s %s [%s]\n%s Daily Growing Degree Days (50/86)" % (
