...
def get_description():
    ...
    d['arguments'] = [
        ...
        dict(type='text', name='year', default=datetime.datetime.now().year, 
...
def plotter(fdict):
    ...
    for i in range(len(bars)):
        ax.text(bars[i].get_x()+0.2, avgs[i] + 1.5, "%.1f" % (avgs[i],), ha='center',
                zorder=2)
        delta = yearlytotals[year-minyear,i] / normal * 100.0 - 20.0
        ax.text(i, max(avgs[i], dlast[i]) + 4, 
                "%s%.1f%%" % ("+" if delta > 0 else "", delta,), 
                ha='center', color='r',bbox=dict(facecolor='white', edgecolor='white'),
                zorder=1)
    
    bars = ax.bar( np.arange(5), dlast, width=0.4, fc='r', label='%s' % (year,))
    for i in range(len(bars)):
        ax.text(bars[i].get_x()+0.2, dlast[i] + 1.5, "%.0f" % (dlast[i],), ha='center')
    
    #ax.text(0.8, 0.8, "Yearly Avg: %.2f" % (normal,),
