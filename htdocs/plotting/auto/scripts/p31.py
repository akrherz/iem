...
def plotter(fdict):
    ...
    rows = []
    for row in cursor:
        rows.append(dict(week=int((row[0]-1)), jump_up=row[1],
                         jump_down=row[2]))
    ...
    ax.text(183, extreme-5,
            ("Maximum Jump in High Temp vs Max High over past %s days"
             ) % (days,),
            color='red', va='center', ha='center', bbox=dict(color='white'))
    ax.text(183, 0-extreme+5,
            ("Maximum Dip in High Temp vs Min High over past %s days"
             ) % (days,),
