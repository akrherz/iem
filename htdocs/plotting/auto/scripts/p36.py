...
def plotter( fdict ):
    ...
    for rownum, row in enumerate(cursor):
        if rownum == 0:
            baseyear = row[0]
            avgs = np.zeros((datetime.datetime.now().year - baseyear + 1, 12))
    ...
    for i in range(12):
        for j in range(12):
            txt = ax.text(i,j, "%s" % ("%.0f" % (matrix[i,j],) if i != j else '-'), 
    ...
    ax.set_title("[%s] %s\nYears that Month was Warmer than other Month" % (station, nt.sts[station]['name']))
    fig.colorbar(res)
