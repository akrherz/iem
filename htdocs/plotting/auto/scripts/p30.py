...
def get_description():
    ...
    desc['arguments'] = [
        ...
        dict(type="select", name='opt', default='monthly', options=PDICT,
        ...
        dict(type='year', name='eyear', default=today.year,
...
def plot_trailing(ax, df, colname):
    ...
    ax.plot(df.index.values[30:], trail, lw=1.5, color='red', zorder=5,
            label='Trailing 30yr')
    ax.axhline(df[colname].mean(), lw=2, color='k', zorder=2, label='Avg')
    ax.text(df.index.values[-1]+2, df[colname].mean(),
            "%.0f" % (df[colname].mean(),),
            ha='left', va='center')
...
def plotter(fdict):
    ...
    if ctx['opt'] == 'monthly':
        ...
        ORDER by year ASC
        """, pgconn, params=(station, month, ctx['eyear']), index_col='year')
    else:
        df = read_sql("""
        SELECT year,  max(high) as max_high,  min(low) as min_low
    ...
    if year in df.index:
        ax[0].scatter(year, df.at[year, 'max_high'], marker='o',
                      color='r', s=10)
    ...
    if year in df.index:
        ax[1].scatter(year, df.at[year, 'min_low'], marker='o',
                      color='r', s=10)
    ...
    if year in df.index:
        ax[2].scatter(year, df.at[year, 'rng'], marker='o',
                      color='r', s=10)
        ax[2].set_title(("Year %s [Hi: %s Lo: %s Rng: %s] Highlighted"
