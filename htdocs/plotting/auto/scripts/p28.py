...
def get_description():
    ...
    d['arguments'] = [
        ...
        dict(type='date', name='date', default='2014/10/15', 
             label='To Date:',
...
def plotter( fdict ):
    ...
    for row in cursor:
        ...
        # right hand
        data[int(row['year'] - baseyear -1), int(row['doy']) + 366] = row['precip']
    ...
    ax.set_title("%s %s\nTrailing Days Precipitation Rank [%s-%s] to %s" % (station, 
                                        nt.sts[station]['name'], baseyear+2,
    ...
    y2 = ax.twinx()
    y2.plot(np.arange(-365,0), totals[::-1], color='r', lw=2, label='This Period')
    y2.plot(np.arange(-365,0), maxes[::-1], color='g', lw=2, label='Maximum')
