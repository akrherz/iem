...
def get_description():
    ...
    desc['data'] = True
    desc['cache'] = 300
    desc['description'] = """This app produces a table of frequencies of
    wind and hail tags used in NWS Warnings."""
    today = datetime.datetime.today() + datetime.timedelta(days=1)

    desc['arguments'] = [
        dict(type='networkselect', name='station', network='WFO',
             default='_ALL', label='Select WFO:', all=True),
        dict(type='state', name='state',
             default='IA', label='Select State:'),
        dict(type='select', name='opt', default='wfo',
             label='Plot for WFO(all option) or State:', options=PDICT),
        dict(type='date', name='date1', optional=True,
             default="2010/04/01",
             label='Start Date Bounds (optional):', min="2010/04/01"),
        dict(type='date', name='date2', optional=True,
