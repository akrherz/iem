...
def get_description():
    ...
    desc[
        "description"
    ] = """Daily plot of observed high and low temperatures
...
def plotter(fdict):
    ...
    if has_data:
        for _, row in df.iterrows():
            ...
            i += 1
        ax.set_ylim(
            np.nanmin([df["low"].min(), df["min_tmpf"].min()]) - 5,
...
if __name__ == "__main__":
    plotter(
        {"month": 4, "year": 2016, "station": "RAYQ1", "network": "CA_AB_DCP"}
    )
