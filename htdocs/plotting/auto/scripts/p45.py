...
def get_description():
    ...
    a month with an overcast sky reported at a given time of the day.
    """
    d['arguments'] = [
        dict(type='zstation', name='zstation', default='DSM', label='Select Station:'),
        dict(type='hour', name='hour', label='Select Hour of Day:', default=12),
...
def plotter( fdict ):
    ...
    cursor.execute("""
        ...
        obs as (
            ...
            and extract(hour from (valid at time zone t.tzname) + '10 minutes'::interval ) = %s
            GROUP by yyyymmdd),
        
        monthly as (
            SELECT substr(o.yyyymmdd,1,4) as year, substr(o.yyyymmdd,5,2) as month,
    ...
    bars = ax.bar(np.arange(1,13)-0.4, monthly, fc='red', ec='red', width=0.4,
           label='Climatology')
    ...
    for i, bar in enumerate(bars):
        if not np.isnan(thisyear[i]):
            ax.text(i+1+0.25, thisyear[i]+1, "%.0f" % (thisyear[i],), ha='center')
    ax.set_ylim(0,100)
