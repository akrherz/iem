...
def plotter( fdict ):
    ...
    cursor.execute("""
     ...
     max(init_expire at time zone 'UTC') from warnings 
    ...
    for row in cursor:
        events.append( (row[3].replace(tzinfo=pytz.timezone("UTC")), 
                        max(row[4],row[5]).replace(tzinfo=pytz.timezone("UTC")), 
                        row[2]) )
    ...
    colors = {'BZ.W': '#FF4500', 'WC.Y': '#AFEEEE', 'WW.Y': '#7B68EE',
          ...
          'HW.A': '#B8860B', 'WS.W': '#FF69B4', 'WS.A': '#4682B4'}
    ...
    for i, e in enumerate(events):
        ...
        ax.text(xpos, i+1, 
                ...
                color=colors.get(types[i], 'k'), ha=align, va='center',
