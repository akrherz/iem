...
def plotter( fdict ):
    ...
    nochange = []
    for row in cursor:
        increase.append( row["%s_greater" % (varname,)] )
        nochange.append( row["%s_unch" % (varname,)] )
        decrease.append( row["%s_lower" % (varname,)] )

    increase = np.array(increase, 'f')
    nochange = np.array(nochange, 'f')
    decrease = np.array(decrease, 'f')
    ...
    ax.bar( np.arange(1,13)-0.4, increase / total * 100.0, bottom=(decrease+nochange)/total * 100.0,  fc='r', label="Increase")
    ...
    for i in range(1,13):
        ...
        txt.set_path_effects([PathEffects.withStroke(linewidth=2,
                                                     foreground="white")])
        txt = ax.text(i, (decrease[i-1] + nochange[i-1])/ total[i-1] * 100.0 + 2, "%.0f" % (
