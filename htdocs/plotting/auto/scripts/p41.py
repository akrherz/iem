...
def plotter( fdict ):
    ...
    ax.plot(p1, p1 * s_slp + s_int, lw=3, color='r', 
            zorder=2, label=r"Fit R$^2$=%.2f" % (
                                                        s_r ** 2,))
    ax.axvline(highlight, zorder=1, color='k')
    y = highlight * s_slp + s_int
    ax.axhline(y, zorder=1, color='k')
    ax.text(p1[1], y, "%.0f $^\circ$F" % (y,), va='center', bbox=dict(color='white'))
    ax.text(highlight, p2[1], "%.0f $^\circ$F" % (highlight,), ha='center', 
            rotation=90, bbox=dict(color='white'))
    ax.set_title("[%s] %s\n%s vs %s %s Quantile-Quantile Plot" % (station, 
            nt.sts[station]['name'], calendar.month_abbr[month2],
