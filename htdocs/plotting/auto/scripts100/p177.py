...
def get_description():
    ...
    desc['arguments'] = [
        ...
        dict(type='datetime', name='sts',
             default=sts.strftime("%Y/%m/%d %H%M"),
             label='Start Time (Central Time):', min="2012/01/01 0000"),
        dict(type='datetime', name='ets',
...
def make_daily_water_change_plot(ctx):
    ...
    """, ctx['pgconn'], params=(ctx['station'], ctx['sts'].date(),
                                ctx['ets'].date()), index_col='valid')
