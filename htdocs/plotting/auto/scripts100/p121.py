...
def get_description():
    ...
    desc['data'] = True
    desc['report'] = True
    desc['description'] = """This report displays the number of cycles between
    ...
    desc['arguments'] = [
        ...
        dict(type="text", name='thres2', default='24-40',
             label="Threshold 2 (lower-upper) (F)"),
        dict(type="text", name='thres3', default='20-44',
             label="Threshold 3 (lower-upper) (F)"),
        dict(type="text", name='thres4', default='14-50',
...
""" % (datetime.date.today().strftime("%d %b %Y"),
       ...
       prs[3][0], prs[3][1], prs[3][0], prs[3][1],
       )

    df = pd.DataFrame({thres1 + 's': 0,
                       thres1 + 'f': 0,
                       thres2 + 's': 0,
                       thres2 + 'f': 0,
                       thres3 + 's': 0,
    ...
    for yr, row in df.iterrows():
        res += ("%s   %-8i%-8i%-8i%-8i%-8i%-8i%-8i%-8i\n"
                     ...
                     row[thres1 + 's'], row[thres1 + 'f'],
                     row[thres2 + 's'], row[thres2 + 'f'],
                     row[thres3 + 's'], row[thres3 + 'f'],
