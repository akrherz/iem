...
def get_description():
    ...
    desc['data'] = True
    desc['cache'] = 600
    desc['description'] = """Some of the warnings that the National Weather
    ...
    desc['arguments'] = [
        dict(type='date', name='date', default=today.strftime("%Y/%m/%d"),
             label='Optional Date to Highlight on Map:', min="2007/07/16",
             max=today.strftime("%Y/%m/%d"), optional=True),
        dict(type='networkselect', name='wfo', default='DMX',
             label='Select Forecast Office:', network='WFO'),
        dict(type='select', name='p', default='TO', options=PDICT,
             label='Warning Type:'),
    ]
    return desc
...
def plotter(fdict):
    ...
    g.ax_joint.set_xticklabels(['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW',
                                'N'])
    if date:
        ...
        g.ax_joint.scatter(df2['tml_direction'],
                           ...
                           color='r', s=50,
                           label=date.strftime("%b %-d, %Y"), zorder=2)
    g.ax_joint.legend()
    g.ax_joint.grid()
