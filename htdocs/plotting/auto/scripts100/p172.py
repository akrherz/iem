...
def get_description():
    ...
    desc['arguments'] = [
        ...
        dict(type='year', name='year3', optional=True, default=(thisyear - 2),
             label='Additional Year to Plot: (optional)'),
        dict(
            type='date', name='sdate', default='2000/01/01',
...
def plotter(fdict):
    ...
    df = read_sql("""
        ...
        ORDER by day ASC
    """, pgconn, params=(
        delta, station, sdate.strftime("%m%d"), sdate.strftime("%m%d"), delta,
        station),
                  index_col='day')
    if df.empty:
    ...
    ax.set_title(("Accumulated Precipitation after %s\n"
                  "[%s] %s (%s-%s)"
                  ) % (sdate.strftime("%-d %B"), station,
