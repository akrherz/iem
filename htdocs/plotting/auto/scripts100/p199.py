...
def get_description():
    ...
    desc = dict()
    desc['data'] = True
    desc['description'] = """This application generates maps of daily ISU
    ...
    desc['arguments'] = [
        ...
        dict(type='date', name='date', default=sts.strftime("%Y/%m/%d"),
             label='Select Date', min="2012/01/01"),
        ]
    return desc
...
def plot5(ctx, col):
    """Daily ET."""
    df = read_sql("""
        SELECT station, """ + col + """_qc from sm_daily WHERE
        valid = %s and """ + col + """_qc is not null
    """, ctx['pgconn'], params=(ctx['date'], ),
                  index_col='station')
    if df.empty:
        raise NoDataFound("No Data Found for This Plot.")
