...
def get_description():
    ...
    desc['arguments'] = [
        ...
        dict(type='text', name='r1', default='50-59',
             label='Range #1 (inclusive)'),
        dict(type='text', name='r2', default='60-69',
             label='Range #2 (inclusive)'),
        dict(type='text', name='r3', default='70-79',
             label='Range #3 (inclusive)'),
        dict(type='text', name='r4', default='80-89',
             label='Range #4 (inclusive)'),
        dict(type='text', name='r5', default='90-99',
...
def plotter(fdict):
    ...
    gdf = df[['cnt1', 'cnt2', 'cnt3', 'cnt4', 'cnt5', 'year']
             ].groupby('year').sum()

    (fig, ax) = plt.subplots(1, 1)
    bars = ax.bar(np.arange(1, 6) - 0.25, gdf.mean().values, width=-0.25,
                  label="Average")
    for i, mybar in enumerate(bars):
        ax.text(mybar.get_x() - 0.35, mybar.get_height() + 0.5,
                "%.1f" % (mybar.get_height(), ))

    bars = ax.bar(range(1, 6), gdf.loc[year].values, width=0.25,
                  label=str(year))
    for i, mybar in enumerate(bars):
        ax.text(mybar.get_x(), mybar.get_height() + 0.5,
                "%.0f" % (mybar.get_height(), ))

    bars = ax.bar(np.arange(1, 6) + 0.25, gdf.loc[date.year].values,
                  width=0.25, label=str(date.year))
    for i, mybar in enumerate(bars):
        ax.text(mybar.get_x(), mybar.get_height() + 0.5,
                "%.0f" % (mybar.get_height(), ))
    ax.grid(True)
    ax.set_xticks(range(1, 6))
    ax.set_xticklabels(["%g - %g" % (one, two) for (one, two) in [r1, r2, r3,
                                                                  r4, r5]])
    ax.legend(loc='best')
    ax.set_ylabel("Days")
