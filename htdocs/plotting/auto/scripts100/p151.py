...
def plotter(fdict):
    ...
    p1agg as (
        ...
        avg(sum_gdd) as gdd from period1 GROUP by station),
    p2agg as (
        SELECT station, avg(total_precip) as precip,
        avg(avg_temp) as avg_temp, avg(avg_high) as avg_high,
        avg(avg_low) as avg_low, avg(sum_sdd) as sdd,
        avg(sum_gdd) as gdd from period2 GROUP by station),
    agg as (
    ...
    """, pgconn, params=[p1syear, p1eyear, tuple(months),
                         p2syear, p2eyear, tuple(months)],
                  index_col=None)
