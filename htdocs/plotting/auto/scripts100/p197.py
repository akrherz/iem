...
LOOKUP = {
    ...
    'corn_poor_verypoor': [
        'CORN', 'CONDITION', ['PCT POOR', 'PCT VERY POOR'],
        'ALL UTILIZATION PRACTICES'
    ],
    'corn_good_excellent': [
        'CORN', 'CONDITION', ['PCT GOOD', 'PCT EXCELLENT'],
        'ALL UTILIZATION PRACTICES'
    ],
}
...
def get_description():
    ...
    desc['cache'] = 3600
    desc['nass'] = True
    desc['description'] = """This generates a map showing USDA NASS weekly
    ...
    desc['arguments'] = [
        dict(type="select", options=PDICT, default="corn_silking", name="var",
...
def get_df(ctx):
    ...
    ctx['title'] = (
        "%s USDA NASS %s" % (date.strftime("%-d %B %Y"), PDICT[varname])
    )
    ctx['subtitle'] = (
