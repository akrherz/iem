...
def get_description():
    ...
    desc['arguments'] = [
        ...
        dict(type='select', options=PDICT, default='fall',
             label='Which metric to compute', name='var'),
        dict(type='int', name='thres', default=32,
             label='Temperature Threshold (F) for initial date of exceedence'),
        dict(type='int', name='thres2', default=60,
             label='Temperature Threshold (F) to count days above/below'),
        dict(type='year', default=today.year, name='year',
...
def plotter(fdict):
    ...
    if varname == 'fall':
        ...
        sql = """
        WITH doy as (
            SELECT year, max(day) from """ + table + """ WHERE station = %s
    ...
    if df2 is not None:
        ax2.scatter(df2['day_of_year'], df2['peak_value'], color='red',
                    zorder=2)
    ax2.grid(True)
    ax2.set_xticks((1, 32, 60, 91, 121, 152, 182, 213, 244, 274,
                   305, 335, 365))
    ...
    ax3.grid(True)
    ax3.set_ylabel("Days %s %.0f" % ('above' if varname == 'fall' else 'below',
                                     thres2))
    ax3.set_xlim(df.index.min() - 1, df.index.max() + 1)
    ax3.axhline(df['count_days'].mean())
    ax3.text(df.index.max() + 1, df['count_days'].mean(),
             "Avg:\n%.1f" % (df['count_days'].mean(),))
    ax3.set_xlabel(("%0.f years with 0 days, %s = %.0f"
                    ) % (len(df[df['count_days'] == 0].index), ctx['year'],
