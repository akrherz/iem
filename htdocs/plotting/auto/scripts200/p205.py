...
def get_description():
    ...
    desc = dict()
    desc['data'] = True
    desc['description'] = """The IEM computes daily summaries of ASOS station
    ...
    desc['arguments'] = [
        ...
        dict(
            type="int", name="max_dwpf_below", default=50, optional=True,
            label="Daily Maximum Dewpoint Below (threshold F): [optional]",
        ),
        dict(
            type="int", name="max_rh_below", default=50, optional=True,
            label="Daily Maximum Relative Humidity Below (%): [optional]",
        ),
        dict(
            type="int", name="avg_smph_below", default=10, optional=True,
            label="Daily Average Wind Speed Below (threshold MPH): [optional]",
        ),
        dict(
            type="text", name="max_tmpf_range", default="70 to 79",
            optional=True,
...
def plotter(fdict):
    ...
    if ctx.get('max_rh_below'):
        params.append((
            "case when max_rh < %.0f "
            "then 1 else 0 end as rh_hit") % (ctx['max_rh_below'], ))
    ...
    if ctx.get('max_tmpf_range'):
        ...
        params.append((
            ...
            "then 1 else 0 end as tmpf_hit") % (tokens[0], tokens[1]))
        labels['tmpf_hit'] = "High Temp %.0fF - %.0fF" % (tokens[0], tokens[1])
    if ctx.get('max_dwpf_below'):
        params.append((
            "case when max_dwpf < %.0f "
            "then 1 else 0 end as dwpf_hit") % (ctx['max_dwpf_below'], ))
    ...
    """, pgconn, params=(station, ctx['network']))
    if df.empty:
    ...
    for col in df.columns:
        ...
        ax.plot(
            gdf.index.values, gdf[col].values * 100., lw=2,
            label=labels[col], color=color)
