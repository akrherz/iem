...
def get_description():
    ...
    desc['arguments'] = [
        dict(type="select", options=PDICT, default="trail_precip_percent",
             name="var", label='Available variables to plot:'),
        dict(type='station', name='station', default="IATDSM",
             label='Select station:', network='IACLIMATE'),
        dict(type='int', name='days', default='31',
             label='Trailing Number of Days (when appropriate):'),
        dict(type='year', default=(today.year - 20),
             label='Start Year of Plot:', name='syear'),
        dict(type='year', default=today.year,
             label='End Year (inclusive) of Plot:', name='eyear'),
...
def plotter(fdict):
    ...
    """, pgconn, params=(ctx['station'], ), index_col='day', parse_dates='day')
    if df.empty:
    ...
    if ctx['var'] == 'trail_precip_percent':
        ...
        df['trail_precip_percent'] = (
            ...
            df['precip_avg'].rolling(ctx['days']).sum() * 100.
        )
    ...
    ax.set_title(
        "[%s] %s (%s-%s)\n%s" % (
            ctx['station'],
            ctx['_nt'].sts[ctx['station']]['name'], ctx['syear'],
            ctx['eyear'], PDICT[ctx['var']].replace("XX", str(ctx['days'])))
    )
