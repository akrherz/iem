...
$y2label = "Volumetic Soil Moisture [%]";
...
$query2 = "SELECT ". $queryData .", to_char(valid, 'yymmdd/HH24') as tvalid from ". $table ." WHERE 
	date(valid) >= ('". $date ."'::date - '1 days'::interval)  ORDER by tvalid ASC LIMIT 480";
...
$graph->SetY2Scale("lin", 0, 700);
$graph->img->SetMargin(40,50,55,90);
$graph->xaxis->SetFont(FONT1,FS_BOLD);
$graph->xaxis->SetTickLabels($xlabel);
$graph->xaxis->SetLabelAngle(90);
$graph->xaxis->SetPos("min");
$graph->title->Set("Solar Rad & Soil Moisture for Ames SCAN Site");
