...
def get_data(network, sts, ets, stations=[]):
    ...
    s = ("station,day,max_temp_f,min_temp_f,max_dewpoint_f,"
         "min_dewpoint_f,precip_in,avg_wind_speed_kts,avg_wind_drct,climo_high_f,climo_low_f,climo_precip_in\n")
    if len(stations) == 1:
        stations.append('ZZZZZ')
    cursor.execute("""SELECT id, day, max_tmpf, min_tmpf, max_dwpf, min_dwpf,
        pday, avg_sknt, vector_avg_drct from summary s JOIN stations t
