...
def fetch_daily(form, cols):
    ...
    for row in cursor:
        valid = row['valid']
        station = row['station']
        high = temperature(row['tair_c_max'], 'C').value('F') if row['tair_c_max'] is not None else -99
        low = temperature(row['tair_c_min'], 'C').value('F') if row['tair_c_min'] is not None else -99
        precip = row['rain_mm_tot'] / 24.5 if row['rain_mm_tot'] > 0 else 0
        et = row['dailyet'] / 24.5 if row['dailyet'] > 0 else 0

        soil04t = temperature(row['tsoil_c_avg'], 'C').value('F') if row['tsoil_c_avg'] is not None else -99
        soil04tn = temperature(row['soil04tn'], 'C').value('F') if row['soil04tn'] is not None else -99
        soil04tx = temperature(row['soil04tx'], 'C').value('F') if row['soil04tx'] is not None else -99

        soil12t = temperature(row['t12_c_avg'], 'C').value('F') if row['t12_c_avg'] is not None else -99
        soil12tn = temperature(row['soil12tn'], 'C').value('F') if row['soil12tn'] is not None else -99
        soil12tx = temperature(row['soil12tx'], 'C').value('F') if row['soil12tx'] is not None else -99

        soil24t = temperature(row['t24_c_avg'], 'C').value('F') if row['t24_c_avg'] is not None else -99
        soil24tn = temperature(row['soil24tn'], 'C').value('F') if row['soil24tn'] is not None else -99
        soil24tx = temperature(row['soil24tx'], 'C').value('F') if row['soil24tx'] is not None else -99

        soil50t = temperature(row['t50_c_avg'], 'C').value('F') if row['t50_c_avg'] is not None else -99
        soil50tn = temperature(row['soil50tn'], 'C').value('F') if row['soil50tn'] is not None else -99
        soil50tx = temperature(row['soil50tx'], 'C').value('F') if row['soil50tx'] is not None else -99
        ...
        speed = row['ws_mps_s_wvt'] * 2.23 if row['ws_mps_s_wvt'] is not None else -99
...
def fetch_hourly(form , cols):
    ...
    for row in cursor:
        valid = row['valid']
        station = row['station']
        tmpf = temperature(row['tair_c_avg'], 'C').value('F') if row['tair_c_avg'] is not None else -99
        relh = row['rh'] if row['rh'] is not None else -99
        solar = row['slrkw_avg'] if row['slrkw_avg'] is not None else -99
        precip = row['rain_mm_tot'] / 24.5 if row['rain_mm_tot'] is not None else -99
        speed = row['ws_mps_s_wvt'] * 2.23 if row['ws_mps_s_wvt'] is not None else -99
        drct = row['winddir_d1_wvt'] if row['winddir_d1_wvt'] is not None else -99
